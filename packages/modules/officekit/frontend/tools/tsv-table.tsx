...
/** TSV table — the CSV Table experience for tab-separated data. */
export default function TsvTableTool() {
  const t = useTranslations("mod-officekit");
  const [text, setText] = useState("");

  useEffect(() => {
    const handed = takePendingFiles()?.[0];
    if (handed && handed.size < 10 * 1024 * 1024) void handed.text().then(setText);
  }, []);
