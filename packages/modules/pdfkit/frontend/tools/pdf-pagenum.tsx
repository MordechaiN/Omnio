"use client";

import { useState } from "react";
import { useTranslations } from "next-intl";
import { rgb, StandardFonts } from "pdf-lib";
import { Badge, Button, Select, SelectContent, SelectItem, SelectTrigger, SelectValue, Label } from "@omnio/ui";
import { pdfFilename } from "../../shared/pages.ts";
import { downloadPdf, loadPdf, PdfDropZone, usePendingPdf, type LoadedPdf } from "../lib/pdf-file.tsx";

const POSITIONS = ["bottomCenter", "bottomEnd", "topEnd"] as const;
type Position = (typeof POSITIONS)[number];

/** Page numbers — "n / total" stamped on every page. */
export default function PdfPagenumTool() {
  const t = useTranslations("mod-pdfkit");
  const [file, setFile] = useState<LoadedPdf | null>(null);
  const [position, setPosition] = useState<Position>("bottomCenter");
  const [failed, setFailed] = useState<"load" | "save" | null>(null);
  const [busy, setBusy] = useState(false);

  async function open(files: File[]) {
    setFailed(null);
    try {
      setFile(await loadPdf(files[0]!));
    } catch {
      setFailed("load");
    }
  }
  usePendingPdf((files) => void open(files));

  async function apply() {
    if (!file) return;
    setBusy(true);
    setFailed(null);
    try {
      const font = await file.doc.embedFont(StandardFonts.Helvetica);
      const pages = file.doc.getPages();
      pages.forEach((page, index) => {
        const { width, height } = page.getSize();
        const label = `${index + 1} / ${pages.length}`;
        const size = 10;
        const textWidth = font.widthOfTextAtSize(label, size);
        const x =
          position === "bottomCenter" ? (width - textWidth) / 2 : width - textWidth - 28;
        const y = position === "topEnd" ? height - 24 : 16;
        page.drawText(label, { x, y, size, font, color: rgb(0.35, 0.35, 0.35) });
      });
      downloadPdf(await file.doc.save(), pdfFilename(file.name, "numbered"));
    } catch {
      setFailed("save");
    } finally {
      setBusy(false);
    }
  }

  return (
    <div className="flex flex-col gap-5">
      <PdfDropZone onFiles={(files) => void open(files)} hasFile={file !== null} />
      {failed === "load" ? (
        <p role="alert" className="text-sm text-danger">{t("ui.errorLoad")}</p>
      ) : null}

      {file ? (
        <>
          <div className="flex flex-wrap items-center gap-2 text-sm">
            <span dir="ltr" className="max-w-60 truncate font-medium">{file.name}</span>
            <Badge variant="neutral">{t("ui.pageCount", { count: file.pageCount })}</Badge>
          </div>
          <div className="flex max-w-xs flex-col gap-1.5">
            <Label htmlFor="pn-pos">{t("ui.pnPosition")}</Label>
            <Select value={position} onValueChange={(next) => setPosition(next as Position)}>
              <SelectTrigger id="pn-pos"><SelectValue /></SelectTrigger>
              <SelectContent>
                {POSITIONS.map((value) => (
                  <SelectItem key={value} value={value}>
                    {t(`ui.pnPositions.${value}` as Parameters<typeof t>[0])}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          {failed === "save" ? (
            <p role="alert" className="text-sm text-danger">{t("ui.errorSave")}</p>
          ) : null}
          <div>
            <Button type="button" onClick={() => void apply()} disabled={busy}>
              {busy ? t("ui.working") : t("ui.pnAction")}
            </Button>
          </div>
          <p className="text-sm text-text-muted">{t("ui.privacy")}</p>
        </>
      ) : null}
    </div>
  );
}
