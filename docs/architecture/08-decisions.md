...
**Status:** Phase 1 deliverable — decisions D1–D7 need founder sign-off; the "settled" section records choices already justified in this package.

Format: each open decision has a recommendation and its reasoning. Silence = the recommendation ships. Overrides are welcome — that's what this document is for.

---

## Open — founder decisions required
...
### D2 · Authentication default for v1
...
Compose matches the self-hosting audience (same crowd running Immich/Paperless). The worker sandbox ([06-security.md](06-security.md) §2) *requires* container isolation — a bare-metal "just run node" mode would silently drop the security model, so it is deliberately unsupported.
